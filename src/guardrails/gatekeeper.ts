 ...
 * 检测试图绕过系统限制的输入
 */
export const jailbreakRule: GateRule = {
  name: "jailbreak",
 ...
 * 检测试图注入恶意指令的输入
 */
export const injectionRule: GateRule = {
 ...
 * 检测并掩码敏感数据
 */
export const sensitiveDataRule: GateRule = {
  name: "sensitive-data",
 ...
 * 检测试图执行系统命令的输入
 */
export const commandInjectionRule: GateRule = {
...
export class Gatekeeper {
   ...
   * 检查输入
   */
  async checkInput(ctx: GateContext): Promise<GateResult> {
   ...
   * 检查输出
   */
  async checkOutput(ctx: GateContext): Promise<GateResult> {
   ...
   * 列出所有规则
   */
  listRules(): { input: string[]; output: string[] } {
    return {
  ...
  private async runRules(ctx: GateContext, rules: GateRule[]): Promise<GateResult> {
  ...
  private log(action: "allow" | "deny", rule: string, reason?: string): void {
