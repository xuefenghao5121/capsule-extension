...
function detectCPU(): { vendor: "intel" | "amd" | "unknown"; flags: string[] } {
  try {
    ...
    for (const line of cpuinfo.split("\n")) {
      if (line.startsWith("vendor_id")) {
        ...
        if (v === "GenuineIntel") vendor = "intel";
        else if (v === "AuthenticAMD") vendor = "amd";
      }
...
function detectSGX(): { available: boolean; version?: "1" | "2" } {
  const { vendor, flags } = detectCPU();
  if (vendor !== "intel") return { available: false };
  
  const hasSGX = flags.includes("sgx");
  const hasSGXLC = flags.includes("sgx_lc");
  
  if (!hasSGX) return { available: false };
...
function detectTDX(): boolean {
  try {
    if (existsSync("/sys/firmware/tdx/tdx_module")) return true;
...
function detectSEV(): { available: boolean; version?: "1" | "2" | "snp" } {
  const { vendor, flags } = detectCPU();
  if (vendor !== "amd") return { available: false };
  
  const hasSEV = flags.includes("sev");
  const hasSEVES = flags.includes("sev_es");
  const hasSEVSnp = flags.includes("sev_snp");
  
  if (hasSEVSnp) return { available: true, version: "snp" };
  if (hasSEVES) return { available: true, version: "2" };
  if (hasSEV) return { available: true, version: "1" };
...
const x86Plugin: SecurityPlugin = {
  name: "x86-security",
  platform: "x86",
  ...
  async init(info: HardwareInfo): Promise<boolean> {
    ...
    if (sgx) console.log(`[Capsule x86] SGX ${sgx.version} detected`);
    if (tdx) console.log("[Capsule x86] TDX detected");
    if (sev) console.log(`[Capsule x86] SEV ${sev.version} detected`);
