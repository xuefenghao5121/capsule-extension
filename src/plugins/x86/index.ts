...
import { SecurityPlugin, HardwareInfo, SecureContext, SecureContextConfig, ExecutionResult } from "../index.js";
...
function detectSGX(): { available: boolean; version?: "1" | "2"; epcSize?: number } {
  const cpu = detectCPU();
...
const x86Plugin: SecurityPlugin = {
  ...
  async detect(): Promise<HardwareInfo> {
    ...
    // 确定推荐的隔离级别和插件
    let recommendedIsolation: "L1" | "L2" | "L2+" | "L3" = "L1";
  ...
  async createSecureContext(config: SecureContextConfig): Promise<SecureContext> {
  ...
  async destroySecureContext(contextId: string): Promise<void> {
