...
import { Capability, SandboxError } from "../types.js";
...
export function createCapabilityTools(sandboxManager: SandboxManager) {
  return {
    capability_check: {
...
- spawn: Create child processes`,
      ...
      async execute(input: z.infer<typeof CheckInputSchema>) {
        ...
        const hasCapability = sandboxManager.hasCapability(
          validated.sandboxId,
          validated.capability as Capability
        );
    ...
    capability_grant: {
      ...
      async execute(input: z.infer<typeof GrantInputSchema>) {
        ...
        for (const cap of validated.capabilities) {
          sandboxManager.grantCapability(validated.sandboxId, cap as Capability);
        }
    ...
    capability_revoke: {
      ...
      async execute(input: z.infer<typeof RevokeInputSchema>) {
        ...
        for (const cap of validated.capabilities) {
          sandboxManager.revokeCapability(validated.sandboxId, cap as Capability);
        }
