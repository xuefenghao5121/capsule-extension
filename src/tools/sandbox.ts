...
export function createSandboxTools(sandboxManager: SandboxManager) {
  return {
    sandbox_create: {
      name: "sandbox_create",
...
- spawn: Create child processes`,
      ...
      async execute(input: z.infer<typeof CreateInputSchema>) {
        const validated = CreateInputSchema.parse(input);

        const sandbox = await sandboxManager.create({
          name: validated.name,
          isolationLevel: validated.isolationLevel as IsolationLevel,
          capabilities: (validated.capabilities ?? ["file_read"]) as Capability[],
          quota: validated.quota as Partial<ResourceQuota>,
    ...
    sandbox_destroy: {
      ...
      async execute(input: z.infer<typeof DestroyInputSchema>) {
