...
export function createAttestationTool(sandboxManager, security) {
    return {
        name: "attestation",
...
- Compliance verification`,
        ...
        async execute(input) {
            ...
            if (validated.action === "generate") {
                return await generateAttestation(validated.sandboxId, validated.nonce, sandboxManager, security);
            }
            else {
...
async function generateAttestation(sandboxId, nonce, sandboxManager, security) {
...
async function verifyAttestation(reportBase64) {
    if (!reportBase64) {
