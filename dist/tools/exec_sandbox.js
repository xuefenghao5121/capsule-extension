 ...
 */
import { z } from "zod";
import { SandboxError, CapabilityDeniedError, QuotaExceededError, } from "../types.js";
...
export function createExecSandboxTool(sandboxManager, security) {
    return {
        name: "exec_sandbox",
...
- trustzone: ARM TrustZone Secure World`,
        ...
        async execute(input) {
            ...
            const { command, args = [], sandboxId, isolationLevel = "L1", securityFeatures = [], timeout = 60000, workspace, env = {}, } = validated;
            ...
            try {
                ...
                const result = await executeWithIsolation(command, args, {
...
async function executeWithIsolation(command, args, options, security) {
...
async function executeDirect(command, args, options) {
...
async function executeProcessSecure(command, args, options, security) {
...
async function executeDocker(command, args, options) {
...
async function executeDockerTEE(command, args, options, security) {
...
async function executeTrustZone(command, args, options, _security) {
