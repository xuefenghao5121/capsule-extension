...
export function createQuotaTools(sandboxManager) {
...
- memoryMB: Memory usage in MB`,
        ...
        quota_record: {
            ...
            async execute(input) {
                ...
                // Update usage
                if (validated.inferences !== undefined)
                    current.inferences += validated.inferences;
                if (validated.tokens !== undefined)
                    current.tokens += validated.tokens;
                if (validated.cpuSeconds !== undefined)
                    current.cpuSeconds += validated.cpuSeconds;
                if (validated.memoryMB !== undefined)
                    current.memoryMB = Math.max(current.memoryMB, validated.memoryMB);
                return {
                    success: true,
                    sandboxId: validated.sandboxId,
                    recorded: {
