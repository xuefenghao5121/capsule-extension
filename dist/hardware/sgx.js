 ...
 */
export async function checkSGXAvailable() {
    const info = {
        available: false,
        version: "None",
        devices: [],
 ...
 */
export async function generateAttestation(data, nonce) {
    const info = await checkSGXAvailable();
    if (!info.available) {
 ...
 */
export async function verifyAttestation(report, signature) {
    try {
 ...
 */
export async function executeInEnclave(command, args = [], options = {}) {
    const info = await checkSGXAvailable();
    if (!info.available) {
        throw new Error("SGX not available");
    }
 ...
 */
async function executeWithGramine(command, args, options) {
 ...
 * Execute with Occlum
 */
async function executeWithOcclum(command, args, options) {
    const { spawn } = require("child_process");
 ...
 */
async function executeInSimulation(command, args, options) {
    const { spawn } = require("child_process");
    return new Promise((resolve) => {
 ...
 * Seal data using SGX
 */
export async function sealData(data, keyId) {
 ...
 */
export async function unsealData(sealedData) {
    try {
